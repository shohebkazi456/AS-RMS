...
<?php
    ...
    class Login extends rms
    {
        ...
        public function checkLogin($user_email,$user_password)
        {
            ...
            $res=mysqli_query($con,"SELECT * FROM user_table WHERE user_email = '$user_email' and user_password = '$user_password'");
            ...
            $data=mysqli_num_rows($res);
                ...
                if($data > 0)
                {
                    ...
                    $query=mysqli_query($con,"SELECT * FROM user_table WHERE user_email = '$user_email' and user_password = '$user_password'");

                    while($res=mysqli_fetch_array($query))
                    {

                        if($res["user_status"] == 'Enable')
                        {

                            if($res["user_password"] == $_POST["user_password"])
                            {
                        ...
                        {
                            echo '<script>alert("Sorry, Your account has been disable, contact Admin");</script>';
                        }
    ...
    {
        $ob->checkLogin($user_email,$user_password);
    }
