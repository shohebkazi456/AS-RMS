...
class rms
{
	public $base_url = 'http://localhost/AS RMS/';
	public $connect;
	public $query;
	public $statement;
	public $cur;
	...
	{
		return date("Y-m-d H:i:s",  STRTOTIME(date('H:i:s')));
	}
	...
	function Get_total_today_sales()
	{
		...
		while($result = mysqli_fetch_array($query))
		{
			...
			{
				return $this->Get_currency_symbol().' ' . $result["total"];
			}
			else
			{
				return '0';
	...
	function Is_set_up_done()
	{
		...
		$res=mysqli_query($con,"SELECT restaurant_id FROM restaurant_table");
